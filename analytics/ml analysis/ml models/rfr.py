...
def Plt_Importance(data, importance, num_features, app, ls, ts, pltPath):
    ...
    plotdata.plot(kind="bar", colormap=colors, rot=60, legend=None)
    ...
    plt.tight_layout()
    plt.savefig(pltPath+'-'+num_features+app+'-'+'featuresofimportance.png',transparent=True,dpi=600)
    # plt.show()
    print(labels[2:])
...
def new_app_fit_predict(gpu_data,x_gpu,y_gpu,p100_data,data,x,y,x_p100,y_p100,x_p100_df,y_p100_df,x_v100_df,y_v100_df,apps_list,ls,ts,ms,num_features,size,pltPath,x_dgemm, y_dgemm, x_stream, y_stream, label, app1, app2):
    ...
    plt.plot([y_dgemm.min(), y_dgemm.max()], [y_dgemm_pred.min(),
                                            y_dgemm_pred.max()], '--', color='red', lw=2,label = app1+' Regression Line')
    y_stream_pred = lm.predict(x_stream)
    plt.plot([y_stream.min(), y_stream.max()], [y_stream_pred.min(),
                                            y_stream_pred.max()], '--', color='yellow', lw=2,label = app2+' Regression Line')
    apps_list = []
    # for a in apps_list:
    ...
    plt.grid(True)
    plt.savefig(pltPath+num_features+'-'+app1+'-'+app2+'-bm-prediction-NEWAPP.png',
    ...
    plt.savefig(pltPath+num_features+'-'+app1+'-'+app2+'-bm_orig_measured-NEWAPP.png',
                transparent=True, bbox_inches='tight',dpi=300)
    # plt.show()
...
def app_fit_predict(p100_data, x_p100, y_p100, x_df, y_df, apps_list, ls, ts, ms, num_features, size, pltPath,label='Power (W)'):
    ...
    for a in apps_list:
        ...
        diff_arrs.append(y_testdf[a] - y_pred)
        print(y_pred.shape)
        ...
        err.append(mape(y_testdf[a], y_pred))
...
def diffs (features,diffs):
    diffs_list = []
    for diff in diffs:
        diffs_list = diffs_list + diff.tolist()
    print ('Features: ',features, 'Total Diffs: ', len(diffs_list))
    print (diffs_list)
