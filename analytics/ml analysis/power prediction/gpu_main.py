...
def Plt_GPU_Mem_Util(p100_data, data):
    ...
    plt_df = plt_df.sort_values(by=['GP100.utilization.gpu [%]','GP100.utilization.memory [%]'])
    
    plt.figure()
    plt.style.use('classic')
    # rgbkymc,RdYlGn,RdYlBu_r
    # print (plt_df.head())
    ax = plt_df.plot.bar(rot=70, colormap='gist_rainbow',figsize=(11, 8))
...
def Plt_Apps_Pwr(gpu_data):
    ...
    sns.boxplot(x="app", y="power.draw [W]", hue='GPU Microarchitecture',data=data, orient="v", palette="hot")
    # Set labels
    ...
    plt.legend(loc='upper left',ncol=2)
    plt.grid(True)
    plt.savefig('C:/rf/apps_pwr_pattern.png',transparent=True, bbox_inches='tight')
...
cls = ['power.draw [W]', 'app','timestamp','utilization.gpu [%]','utilization.memory [%]','clocks.current.sm [MHz]','temperature.gpu']
# ,'die_size','transistors','cores' ,,
n = 0
n_features = 7
for i in range(n_features):
    ...
    # rfr.crossarch_app_fit_predict(gpu_data,x_gpu,y_gpu,p100_data,data,x,y,x_p100,y_p100,x_p100_df,y_p100_df,x_v100_df,y_v100_df,apps_list,ls,ts,ms,num_features,size,pltPath)
