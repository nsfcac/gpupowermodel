...
def Plt_Importance (p100_data,importance,num_features,app,ls,ts,pltPath):
    vals = []
    ...
    plt.tight_layout()
    plt.savefig(pltPath+app+'-'+num_features+'featuresofimportance.png')
    # plt.show()
    print (labels[2:])
...
def crossarch_app_fit_predict(gpu_data,x_gpu,y_gpu,p100_data,data,x,y,x_p100,y_p100,x_p100_df,y_p100_df,x_v100_df,y_v100_df,apps_list,ls,ts,ms,num_features,size,pltPath):
    ...
    # get importance
    # importance = lm.coef_
    # Plt_Importance(data,importance,num_features,'CROSSARCH',ls,ts)
    ...
    for a in apps_list:
        ...
        p_app=plt.scatter(y_p100_df[a], y_pred, color=next(colors), linewidths = 1, marker =markers[mrk],  edgecolor =None,  s = ms)
        mrk += 1
    ...
    plt.suptitle(chr(37)+' MAE= Min: '+str(round(min(err),2))+',   Mean: '+str(round(np.mean(np.array(err)),2))+',   Max: '+str(round(max(err),2))+',   Std: '+str(round(np.std(np.array(err)),2)), fontsize=16,y=0.95)
    plt.tight_layout()
...
def global_fit_predict(data,x_p100,y_p100,x_df,y_df,apps_list,ls,ts,ms,num_features,size,pltPath):   
    ...
    # get importance
    # importance = lm.coef_
    # Plt_Importance(data,importance,num_features,'GLOBAL',ls,ts)
    ...
    plt.plot([y_test.min(), y_test.max()], [y_test_pred.min(), y_test_pred.max()], '--',color = 'red', lw=2)

    for a in apps_list:
        y_pred = lm.predict(x_test_df[a])
        p_app=plt.scatter(y_test_df[a], y_pred, color=next(colors), linewidths = 1, marker =markers[mrk],  edgecolor =None,  s = ms)
        mrk += 1
        plt_apps.append(p_app)
        avg_pred_pwr.append(np.mean(y_pred))
    ...
    plt.grid(True)
    # plt.tick_params(axis='both', which='major', labelsize=12)
    plt.suptitle(chr(37)+' MAE= Min: '+str(round(min(mae_list),2))+',   Mean: '+str(round(np.mean(np.array(mae_list)),2))+',   Max: '+str(round(max(mae_list),2))+',   Std: '+str(round(np.std(np.array(mae_list)),2)), fontsize=16,y=0.95)
    plt.tight_layout()
...
def app_fit_predict(p100_data,x_p100,y_p100,x_df,y_df,apps_list,ls,ts,ms,num_features,size,pltPath):   
    ...
    for app in apps_list:
        ...
        # lm = SVR(kernel='rbf', C=100, gamma=0.1, epsilon=.1) # worst 40%
        # lm = SVR(kernel='linear', C=100, gamma='auto') # similar to linear
        lm = SVR(kernel='poly', C=100, gamma='auto', degree=1, epsilon=.1, coef0=1)
        lm.fit(x_train,y_train)
        lms[app] = lm
    ...
    plt.suptitle(chr(37)+' MAE= Min: '+str(round(min(mae_list),2))+',   Mean: '+str(round(np.mean(np.array(mae_list)),2))+',   Max: '+str(round(max(mae_list),2))+',   Std: '+str(round(np.std(np.array(mae_list)),2)), fontsize=16,y=0.95)
    plt.tight_layout()
    plt.savefig(pltPath+num_features+'_SVR_bm_orig_measured-APP.png')
    # plt.show()
